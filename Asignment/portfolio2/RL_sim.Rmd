...
softmax_func <-  function(x, # A list with the values corresponding to each hand
                          tau # inverse heat: How likely the agent is to stick with the choice predicted by the RL strategy
                          ){
  exps <- exp(x/tau)
  probs <- exps/sum(exps)
  
  
  # OLD: prob <- 1/(1 + exp(-tau * x))
...
results <- tibble('Agent1_choice' = rep(NA,trials), 
...
- Softmax??? Why does it look like this
- Choice -> Can we do it this way with rbinom??
- Feedback? Is it 0 or 1 or -1 and 1



input =  prevchoice, reward
  V_t = V_t-1 + alpha*PE
