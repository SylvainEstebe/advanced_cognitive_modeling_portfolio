...
### loading the model:
```{r}
file <- file.path("/Users/laura/Desktop/AdvCogModeling1/Stan_Models/REL2_model.stan")
...
stan_data <- list(
  trials = n,
  choiceREL = dat$Agent1_choice[1:n], ### rigth now just taking the same length as feedback, to see if it will even work
  feedback = dat$feedback[1:n], # trying to get around the last NA by removing it 
  initialValue = c(dat$left_v1[1], dat$right_v2[1])
)
```
...
```{r}
samples$save_object("/Users/laura/Desktop/advanced_cognitive_modeling_portfolio1/Asignment/portfolio2/simmodels/REL_first_try.rds")
